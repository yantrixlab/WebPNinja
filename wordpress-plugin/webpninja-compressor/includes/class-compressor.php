...
class WebPNinja_Compressor {
	...
	public function compress_attachment( $attachment_id, $metadata = null ) {
		...
		if ( ! in_array( $mime, self::MIMES, true ) ) {
			return $this->save_result( $attachment_id, 'skipped', 0, 0, 0, __( 'Unsupported file type', 'yantrixlab-image-compressor' ) );
		}

		$engine = self::engine();
		if ( ! $engine ) {
			return $this->save_result( $attachment_id, 'failed', 0, 0, 0, __( 'Neither Imagick nor GD is available on this server', 'yantrixlab-image-compressor' ) );
		}
		...
		if ( ! $paths ) {
			return $this->save_result( $attachment_id, 'failed', 0, 0, 0, __( 'Image file not found on disk', 'yantrixlab-image-compressor' ) );
		}

		if ( $this->is_animated( $paths[0], $mime ) ) {
			return $this->save_result( $attachment_id, 'skipped', 0, 0, 0, __( 'Animated image — left untouched to keep the animation', 'yantrixlab-image-compressor' ) );
		}
		...
		$this->save_result( $attachment_id, 'failed', 0, 0, 0, __( 'Did not finish — the server may have run out of memory or time', 'yantrixlab-image-compressor' ) );
